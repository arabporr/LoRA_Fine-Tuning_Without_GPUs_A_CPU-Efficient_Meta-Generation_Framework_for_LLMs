...
def distance_metric(index_1, index_2):
    ...
    JS_divergence = 0.5 * np.sum(f1_hist * np.log(f1_hist / m_hist)) + 0.5 * np.sum(
...
def Distance_Calculator(first_index: int) -> str:
    distances_metric_dir = os.path.join(distances_dir, "JS")
...
def JS_run():
    ...
    All_distances_file_location = os.path.join(distances_dir, "JS_all_distances.pt")
    ...
    #### End of Script Print
    print(40 * "*")
    print("JS DISTANCE CALCULATIONS FINISHED SUCCESSFULLY")
